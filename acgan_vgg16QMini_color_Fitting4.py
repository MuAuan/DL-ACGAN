...
def getDataSet():
    ...
    for i in range(0,9):
        ...
        for j in range(cutNum):
            ...
            img = image.load_img(path+str(i)+"/"+imgList[j], target_size=(128,128))
...
if __name__ == '__main__':
    ...
    discriminator.load_weights('params_discriminator_cifar10_epoch_200.hdf5')
    ...
    generator.load_weights('params_generator_cifar10_epoch_200.hdf5')
    ...
    X_train=x_train[0:3960]  #.transpose(0,2,3,1) 1760
    print("X_train.shape1",X_train.shape)
    X_test=x_test[0:360]   #.transpose(0,2,3,1) 160
    y_train=y_train[0:3960].reshape(1,3960)[0] #1750
    y_test=y_test[0:360].reshape(1,360)[0]
    
    for i in range(13):
        plt.figure(num=None, figsize=(10, 10), dpi=50)
        plt.imshow(X_train.transpose(0,2,3,1)[i][0:127][0:127])
        print(y_train[i])
        plt.pause(1)
        plt.close()
    ...
    for epoch in range(epochs):
        ...
        if epoch%10==0:
            ...
            noise = np.random.uniform(-1, 1, (fig_sc*fig_sc, latent_size))

            sampled_labels = np.array([
                [i] * fig_sc for i in range(fig_sc)
            ]).reshape(-1, 1)
            ...
            for j in range(fig_sc):
                for i in range(fig_sc):
                    plt.subplot(fig_sc,fig_sc,1+i+fig_sc*j)
                    plt.imshow(generated_images.transpose(0,2,3,1)[i+fig_sc*j][0:127][0:127],interpolation='bilinear') 
                    plt.axis('off')
                    plt.subplots_adjust(hspace = .001)
                    #plt.tight_layout()
            plt.pause(3)
            plt.savefig('plot_epoch_{0:03d}_akb_generated.png'.format(epoch), dpi=60)
            plt.close()
